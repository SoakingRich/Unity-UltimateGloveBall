    ...
    public partial class OvrAvatarEntity
    {
        ...
        private abstract class EntityAnimatorBase
        {
                ...
                in CAPI.ovrAvatar2EntityRenderState renderState)
            {
                ...
                if (Entity._isMorphTargetsEnabled && (skeletalAnimation || morphAnimation))
                {
            ...
            List<OvrAvatarSkinnedRenderable> skinnedOut, List<PrimitiveRenderData> renderDataOut)
        {
            foreach (var primRenderables in _visiblePrimitiveRenderers)
            {
                foreach (var primRenderable in primRenderables)
                {
                    ...
                    if (skinnedRenderable == null || !skinnedRenderable.enabled)
                    {
                        continue;
                    }
        ...
        private void SamplePrimitivesSkinningOrigin(in CAPI.ovrAvatar2EntityRenderState renderState)
        {
            ...
            var renderStateIndices = new NativeArray<UInt32>((int)renderState.primitiveCount, Allocator.Temp,
                NativeArrayOptions.UninitializedMemory);
            var renderStates = new NativeArray<CAPI.ovrAvatar2PrimitiveRenderState>((int)renderState.primitiveCount,
            ...
            unsafe
            {
                var result = CAPI.ovrAvatar2Render_GetPrimitiveRenderStatesByIndex(entityId,
                    renderStateIndices.GetPtr<UInt32>(), renderStates.GetPtr<CAPI.ovrAvatar2PrimitiveRenderState>(),
                    renderState.primitiveCount);
                if (result != CAPI.ovrAvatar2Result.Success)
                {
                    result.LogError("GetPrimitiveRenderStatesByIndex", logScope, this);
                    return;
                }
            }
