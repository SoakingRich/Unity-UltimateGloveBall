...
float3 avatar_computeSpecular(float3 normal, float3 worldViewDir, avatar_Light light, float3 lightDirection, avatar_LightLookParams lightLookParams, float roughness, float exposure, avatar_FragOptions options)
{
    ...
    float _3959 = sd.NdotV;
    float _3964 = sd.invRoughPow4;
    float _3967 = sd.roughPow2;
    float ggx = (NdotL * sqrt(((_3959 * _3959) * _3964) + _3967)) + (_3959 * sqrt(((NdotL * NdotL) * _3964) + _3967));
    float t = 1.0f / (1.0f - ((NdotH * NdotH) * _3964));
...
float _noise(float3 p)
{
    ...
    float _2466 = d_1.z;
    float4 o3 = (frac(perm(c + 1.0f.xxxx) * 0.024390242993831634521484375f) * _2466) + (frac(perm(c) * 0.024390242993831634521484375f) * (1.0f - _2466));
    float _2478 = d_1.x;
    float2 o4 = (o3.yw * _2478) + (o3.xz * (1.0f - _2478));
    float _2490 = d_1.y;
    return (o4.y * _2490) + (o4.x * (1.0f - _2490));
}
...
float3 computeSpecularHighlight(float anisotropicIntensity, float roughness, float3 L, float3 E, float3 t, float3 tangent, float3 bitangent, float offset)
{
    ...
    {
        float3 _4189 = E + L;
        float3 isotropicH = normalize(_4189);
        H = normalize(_4189 - (tangent * dot(_4189, tangent)));
...
float4 avatar_finalOutputColor(avatar_FragmentInput i, avatar_FragmentOutput o)
{
    ...
    if (i.debugMode == 2)
    {
        float3 _4506 = i.material.occlusion.xxx;
        color = float4(_4506.x, _4506.y, _4506.z, color.w);
    }
    if (i.debugMode == 3)
    {
        float3 _4517 = ConvertOutputColorSpaceFromSRGB(i.material.roughness.xxx);
        color = float4(_4517.x, _4517.y, _4517.z, color.w);
        if (i.options.enableAnyHair)
        {
            float3 _4527 = ConvertOutputColorSpaceFromSRGB(i.material.hair_material.flow_sample.xxx);
            color = float4(_4527.x, _4527.y, _4527.z, color.w);
        }
    }
    if (i.debugMode == 4)
    {
        float3 _4538 = ConvertOutputColorSpaceFromSRGB(i.material.metallic.xxx);
        color = float4(_4538.x, _4538.y, _4538.z, color.w);
    }
    if (i.debugMode == 5)
    {
        float3 _4548 = i.material.thickness.xxx;
        color = float4(_4548.x, _4548.y, _4548.z, color.w);
    }
    if (i.debugMode == 6)
    {
        float3 _4559 = (i.geometry.normal * 0.5f) + 0.5f.xxx;
        color = float4(_4559.x, _4559.y, _4559.z, color.w);
    }
    if (i.debugMode == 19)
    {
        float3 _4570 = (i.geometry.tangentSpace.tangent * 0.5f) + 0.5f.xxx;
        color = float4(_4570.x, _4570.y, _4570.z, color.w);
    }
    if (i.debugMode == 20)
    {
        float3 _4581 = (i.geometry.tangentSpace.bitangent * 0.5f) + 0.5f.xxx;
        color = float4(_4581.x, _4581.y, _4581.z, color.w);
    }
    if (i.debugMode == 7)
    {
    ...
    if (i.debugMode == 10)
    {
        float3 _4617 = o.p_specular + o.p_diffuse;
        color = float4(_4617.x, _4617.y, _4617.z, color.w);
    }
    if (i.debugMode == 11)
    {
    ...
    if (i.debugMode == 13)
    {
        float3 _4643 = o.a_specular + o.a_diffuse;
        color = float4(_4643.x, _4643.y, _4643.z, color.w);
    }
    if (i.debugMode == 14)
    {
    ...
    if (i.debugMode == 21)
    {
        float3 _4685 = i.ambient_occlusion.xxx;
        color = float4(_4685.x, _4685.y, _4685.z, color.w);
    }
    if (i.debugMode == 18)
    {
...
void frag_Fragment_main()
{
    ...
    float _5462 = ormt.x;
    i.material.occlusion = lerp(1.0f, _5462, u_OcclusionStrength);
    i.material.roughness = ormt.y * u_RoughnessFactor;
    float _5472 = ormt.z;
    i.material.metallic = _5472 * u_MetallicFactor;
    ...
    float _5489 = ormt.w;
    i.material.thickness = _5489 * u_ThicknessFactor;
    ...
    i.material.hair_material.ao_intensity = _5462;
    ...
    i.material.hair_material.shift = _5472;
    i.material.hair_material.blend = clamp(_5489 * 2.0f, 0.0f, 1.0f);
    i.material.hair_material.aniso_blend = clamp((_5489 - 0.5f) * 2.0f, 0.0f, 1.0f);
    if (i.options.enableAnyHair)
    {
    ...
    for (int lightIdx = 0; lightIdx < 10; lightIdx++)
    {
        if (lightIdx < i.lightsCount)
        {
            ...
            if (light.eyeGlint == 0)
            {
                float3 _5862 = avatar_computeDiffuse(i.geometry, light, i.material, diffuseWrap);
                t = _5862;
            }
            else
            {
    ...
    float3 _6119 = -i.geometry.worldViewDir;
    OvrGetUnityDiffuseGlobalIllumination(i.lights[0].color, i.lights[0].direction, i.geometry.positionInWorldSpace, _6119, 1.0f, i.ambient_color, 1.0f - i.material.roughness, i.material.metallic, i.material.occlusion, i.material.base_color, i.geometry.normal, 0.0f.xxx, diffuse);
    avatar_AmbientLighting ambient;
    ambient.diffuse = diffuse;
    float3 diffuse0 = 0.0f.xxx;
    float3 specular_1 = 0.0f.xxx;
    ...
    OvrGetUnityGlobalIllumination(i.lights[0].color, i.lights[0].direction, i.geometry.positionInWorldSpace, _6119, 1.0f, i.ambient_color, 1.0f - roughness, metallic, i.material.occlusion, i.material.base_color, normal, o_1.p_specular, diffuse0, specular_1);
    ambient.specular = specular_1;
    ...
    float3 _6241 = ambient.specular * f;
    bool3 _6242 = i.options.enableEyeGlint.xxx;
    o_1.a_specular = float3(_6242.x ? _6241.x : 0.0f.xxx.x, _6242.y ? _6241.y : 0.0f.xxx.y, _6242.z ? _6241.z : 0.0f.xxx.z);
    ...
    avatar_FragmentOutput _5982 = o_1;
    if (i.options.enableSkin || i.options.enableAnyHair)
    {
        o_1.p_diffuse = (punctualDiffuseWrap * float3(1.0f, 0.300000011920928955078125f, 0.20000000298023223876953125f)) + (_5982.p_diffuse * (1.0f.xxx - float3(1.0f, 0.300000011920928955078125f, 0.20000000298023223876953125f)));
    ...
    avatar_FragmentOutput _6005 = o_1;
    if (i.options.enableRimLight)
    {
        rimLight = 0.0f.xxx + avatar_addRimLight(i.geometry, i.material);
    }
    float3 finalColor_1 = ((((((0.0f.xxx + ((_6005.p_diffuse + _6005.a_diffuse) * (i.material.base_color * (i.material.occlusion * ((true && (isEyeGlintLightUsed > 0)) ? 1.0f : (1.0f - i.material.metallic)))))) + rimLight) + (_5982.p_specular + _5982.a_specular)) + _6005.subSurfaceColor) * i.material.exposure) + glintSpecular) + glintAmbient;
    ...
    float4 _5604 = finalColor;
    finalColor = float4(_5604.xyz.x, _5604.xyz.y, _5604.xyz.z, _5604.w);
    _21 = float4(_5604.xyz.x, _5604.xyz.y, _5604.xyz.z, _5604.w);
}
