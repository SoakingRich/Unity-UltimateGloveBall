...
namespace Oculus.Avatar2
{
    public partial class OvrAvatarEntity : MonoBehaviour
    {
        ...
        {
            return OvrAvatarManager.Instance.EnableFastLoadAvatar &&
...
#pragma warning restore 618
        ...
        */
        ...
        private IEnumerator<OvrTime.SliceStep> LoadAsync_BuildSkeletonAndPrimitives()
        {
            ...
            if (OvrAvatarManager.Instance &&
                OvrAvatarManager.Instance.UseTimeSlicedBuildSkeletonAndPrimitive)
            {
        ...
        {
            bool UseTimeSlicedBuildSkeletonAndPrimitive = OvrAvatarManager.Instance ?
                OvrAvatarManager.Instance.UseTimeSlicedBuildSkeletonAndPrimitive : false;
            ...
            if (UseTimeSlicedBuildSkeletonAndPrimitive && OvrTime.ShouldHold)
            {
                yield return OvrTime.SliceStep.Hold;
            }
        ...
        private IEnumerator<OvrTime.SliceStep> BuildNewPrimitiveRenderablesASync(NativeArray<UInt32> newRenderIndices)
        {
            ...
            foreach (var newRenderIndex in newRenderIndices)
            {
                ...
                if (newRenderable != null)
                {
                    ...
                    if (UseTimeSlicedBuildSkeletonAndPrimitive && OvrTime.ShouldHold) { yield return OvrTime.SliceStep.Hold; }
...
#pragma warning restore 618
        ...
        private IEnumerator<OvrTime.SliceStep> LoadAsyncCoroutine_BuildPrimitives_Internal()
        {
            ...
            do
            {
                ...
                {
                    if (result.newRenderIndices != null && !LoadSync_BuildPrimitives_Internal(in result))
                    {
                        // Retry next frame
                        yield return OvrTime.SliceStep.Wait;
                        // - from the top!
        ...
        private bool LoadSync_BuildPrimitives_Internal(in CheckPrimitivesResult result)
        {
            if (QueryEntityRenderState(out var renderState))
            {
                    ...
                    , Allocator.Temp, NativeArrayOptions.UninitializedMemory))
                {
                    ...
                    var task = LoadASync_BuildPrimitives_Internal(newRenderArray);
                    // Run everything, it should not yield
                    while (task.MoveNext()) {}
                    if (BuildNewPrimitiveRenderablesAsyncBuildAll)
                    {
                        // Update visibility flags
                    ...
                    return BuildNewPrimitiveRenderablesAsyncBuildAll;
                }
            }
            return false;
        }
        ...
        private IEnumerator<OvrTime.SliceStep> LoadASync_BuildPrimitives_Internal(NativeArray<UInt32> newRenderIndices)
        {
            OvrAvatarLog.LogVerbose($"Starting BuildPrimitives", logScope, this);

            var task = BuildNewPrimitiveRenderablesASync(newRenderIndices);
            while (task.MoveNext())
            {
                yield return task.Current;
            }
        ...
        private void CheckLoadedAssets()
        {
            ...
            if (hasDefaultModel || hasUserModel)
            {
                ...
                OvrAvatarLog.AssertConstMessage(
                    SkeletonJointCount > 0, "Skeleton has no joints!", logScope, this);
...
#pragma warning restore 618
        ...
        private void SetCreationContext()
        {
            var shaderIdentifier = OvrAvatarManager.Instance.ShaderManager.GetDefaultShaderIdentifier();

            string jsonString = "{\"skinning_type\": " + (int) SkinningType + ", \"shader_file_type\": " + shaderIdentifier + "}";

            CAPI.ovrAvatar2Entity_SetCreationContext(entityId, jsonString);
        }
