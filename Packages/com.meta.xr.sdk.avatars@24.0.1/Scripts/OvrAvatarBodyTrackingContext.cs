...
namespace Oculus.Avatar2
{
    ...
    public sealed class OvrAvatarBodyTrackingContext : OvrAvatarBodyTrackingContextBase, IOvrAvatarNativeBodyTracking
    {
        ...
        public static OvrAvatarBodyTrackingContext Create(bool runAsync, bool enableHipLock)
        {
            OvrAvatarBodyTrackingContext context = null;
            try
            {
                context = new OvrAvatarBodyTrackingContext(runAsync, enableHipLock);
            }
            catch (Exception)
            {
                context?.Dispose();
        ...
        private OvrAvatarBodyTrackingContext(bool runAsync, bool enableHipLock)
        {
            var createFlags = CAPI.ovrAvatar2BodyProviderCreateFlags.None;
            if (runAsync) createFlags |= CAPI.ovrAvatar2BodyProviderCreateFlags.RunAsync;
            if (enableHipLock) createFlags |= CAPI.ovrAvatar2BodyProviderCreateFlags.EnableHipLock;
            if (!CAPI.ovrAvatar2Body_CreateProvider(createFlags, out _context)
        ...
        {
            return CAPI.ovrAvatar2Body_RestartTracking(_context)
                .EnsureSuccess("ovrAvatar2Body_RestartTracking", logScope);
        }
