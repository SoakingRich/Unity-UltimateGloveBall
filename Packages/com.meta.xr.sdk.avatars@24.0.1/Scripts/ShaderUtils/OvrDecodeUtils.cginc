...
// With sign extension
int OvrBitfieldExtractWithSign(int data, uint offset, uint numBits) {
...
// Unpack 2 16-bit signed integers
int2 OvrUnpackInt2x16(uint packed_values) {
  const int y = OvrBitfieldExtractWithSign(packed_values, 16, 16);
  const int x = OvrBitfieldExtractWithSign(packed_values, 0, 16);

  return int2(x, y);
}

// Unpack 2 16 bit SNorm [-1, 1] entries (packed in a single 32 bit uint)
float2 OvrUnpackSnorm2x16(uint packed_values) {
  const int2 non_normalized = OvrUnpackInt2x16(packed_values);

  // Convert from -32768 -> 32767 to -1 -> 1
  const float inv = 1.0 / 32767.0;

  float2 unpacked = non_normalized * inv;
  unpacked = clamp(unpacked, -1.0, 1.0);
...
float4 OvrUnpackSnorm4x10_10_10_2(int four_packed_values) {
  int4 unpackedInt;
  unpackedInt.x = OvrBitfieldExtractWithSign(four_packed_values, 0, 10);
  unpackedInt.y = OvrBitfieldExtractWithSign(four_packed_values, 10, 10);
  unpackedInt.z = OvrBitfieldExtractWithSign(four_packed_values, 20, 10);
  unpackedInt.w = OvrBitfieldExtractWithSign(four_packed_values, 30, 2);
...
float3 OvrUnpackSnorm3x10_10_10_2WithBonusScale(int packed_value) {
  ...
  // The bonus scale here works by basically making the
  // "simulated N bit encoding" be 2x the size of the
  // "simulated N-1 bit encoding". This is done this way
  // so that the same logic can apply if this
  // data was coming from a hardware decoder that just spits
  // out floats.
...
float3 OvrUnpackSnorm3x10_10_10_2WithBonusScale(in ByteAddressBuffer data_buffer, uint address) {
  return OvrUnpackSnorm3x10_10_10_2WithBonusScale(OvrLoadUint(data_buffer, address));
}
...
uint OvrUnpackUint1x16NonAligned(in ByteAddressBuffer dataBuffer, uint byteOffset) {
  // The byte offset here is not assumed to be on a 4 byte boundary
  // Need to calculate the 4 byte aligned address due to ByteAddressBuffers only
  // being able to load 4 bytes at a time (ByteAddressBuffer = bag of uints)
  // So, this function needs to load a particular uint and then separate out the uint16s

  // Round down to multiple of 4
  const uint address = byteOffset / SIZE_OF_UINT * SIZE_OF_UINT;

  const uint2 twoUints = OvrUnpackUint2x16(dataBuffer, address);

  // Decide which of the 2 to send back
  return twoUints[(uint)(byteOffset != address)];
}
...
// 2x 32-bit uint -> 3x 16-bit signed normalized
float3 OvrUnpackSnorm3x16(in ByteAddressBuffer data_buffer, uint address) {
  uint2 packed_data = OvrLoadUint2(data_buffer, address);
  float2 xy = OvrUnpackSnorm2x16(packed_data.x);
  float z = OvrUnpackSnorm2x16(packed_data.y).x;
...
uint OvrUnpackUint1x8NonAligned(in ByteAddressBuffer dataBuffer, uint byteOffset) {
  // The byte offset here is not assumed to be on a 4 byte boundary
  // Need to calculate the 4 byte aligned address due to ByteAddressBuffers only
  // being able to load 4 bytes at a time (ByteAddressBuffer = bag of uints)
  // So, this function needs to load a particular uint and then separate out the uint8s

  // Round down to multiple of 4
  const uint address = byteOffset / SIZE_OF_UINT * SIZE_OF_UINT;
  const uint4 fourUints = OvrUnpackUint4x8(dataBuffer, address);

  // Decide which of the 4 to send back
  return fourUints[byteOffset - address];
}
...
float3 OvrUnpackSnorm3x10_10_10_2WithBonusScale(
    in StructuredBuffer<uint> data_buffer,
    uint address) {
  return OvrUnpackSnorm3x10_10_10_2WithBonusScale(OvrLoadUint(data_buffer, address));
}
