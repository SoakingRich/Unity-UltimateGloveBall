...
namespace Oculus.Avatar2
{
    ...
    public partial class OvrAvatarEntity : MonoBehaviour
    {
        ...
        // quality level is not changable at runtime, and therefore the _activeQuality is private.
        // its value is set via SetActiveQuality at startup, and gets its value from _creationInfo.renderFilters.quality
        private CAPI.ovrAvatar2EntityQuality _activeQuality = CAPI.ovrAvatar2EntityQuality.Standard;
...
#pragma warning restore 618
        ...
        **/
        ...
        {
            featureFlags.AddExperimentalFeatures();
        }
                ...
                else
                {
                    ...
                    if (result.newRenderIndices != null)
                    {
                            ...
                            , Allocator.Temp, NativeArrayOptions.UninitializedMemory))
                        {
                            ...
                            var task = LoadASync_BuildPrimitives_Internal(newIdxArray);
                            while (task.MoveNext()) { } // Run everything
                        }
                    }
...
#pragma warning restore 618
        ...
        protected void SetActiveSubMeshInclusion(CAPI.ovrAvatar2EntitySubMeshInclusionFlags subMeshInclusion)
        {
            ...
            if(subMeshInclusion != CAPI.ovrAvatar2EntitySubMeshInclusionFlags.All &&
               subMeshInclusion != CAPI.ovrAvatar2EntitySubMeshInclusionFlags.All_Exclusive)
            {
         ...
         */
        protected CAPI.ovrAvatar2EntityQuality GetActiveQuality() => _activeQuality;
            ...
            if (result.IsSuccess())
            {
                ...
                foreach (PrimitiveRenderData[] renderDatas in _visiblePrimitiveRenderers)
                {
                    foreach (PrimitiveRenderData renderData in renderDatas)
                    {
                        ...
                        if (renderer != null)
                        {
                            bool enableNormalMap = (_activeQuality <= CAPI.ovrAvatar2EntityQuality.Standard);
                            bool enablePropertyHairMap = (_activeQuality <= CAPI.ovrAvatar2EntityQuality.Standard);
                            if (enableNormalMap)
                            {
